    ...
    switch (sel) {
        case '1':
            while(1) {
                ...
                gotoxy(mid,down+2);printf("User name::\t");gets(username);
                gotoxy(mid,down+4);printf("Password::\t");get_password(password);
                //action to check user name and password
                is_member = 1;

                if(confirm_member()) {
                    break;
                } else {
                    MessageBox(0,"Invalid User name Or Password:: Please Try Again!","Error",0);
                    strcpy(username,"");
                    strcpy(password,"");
                    log_stat = 0;
                }
