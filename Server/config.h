...
/*
	File:- config.h
	Desc:- contains all the variable declarations,functions declarations,constants definition...
*/
...
#define _WIN32_WINNT 0x0500 //constant for console resizing (redefinition)
...
#include <winsock2.h>   // contains functions for socket programming
#include <process.h>
...
struct client c[NUM_CLIENTS]; //structure client c[n]; if multiple clients

int clients[NUM_CLIENTS];
COORD coord = {0, 0}; // sets coordinates to 0,0
bool SOCKET_START = false;  //TRUE if socket is already initialized
bool login_server = false;  // TRUE if server is already logged in
...
int get_code(void);  //get current key code of entered key
...
void clearArea(int left,int top,int right,int bottom);  //Clears the certain area of screen with whitespace
...
bool flag_newMsg;
bool flag_reset;   //if server reseted this flag, all disconnected clients are cleared up
