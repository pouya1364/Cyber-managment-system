...
void disconnect_client(int x)
{
    if(c[x].connected==true)
    {
        ...
        printf("\couldn't disconnect client:");
}
...
//initialization of windows socket
void start_socket()
{
    WSADATA wsadata;   //initiate the use of ws2_32.dll
    if(WSAStartup(MAKEWORD(2,0),&wsadata)!=0)   //MAKEWORD(2,0) makes request for version 2.0 of win-sock on system
    {
